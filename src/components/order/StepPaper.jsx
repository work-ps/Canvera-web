      ...
      {config.lamination && (
        <section className="oc-section">
          ...
          <div style={{ marginTop: 'var(--space-4)' }}>
            <button
              className="oc-faq-toggle"
              type="button"
              onClick={() => setFaqOpen(prev => !prev)}
              aria-expanded={faqOpen}
            >
              Not sure which paper?
              <svg viewBox="0 0 16 16" fill="none" width="14" height="14" style={{ transform: faqOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
          ...
          {faqOpen && (
            <div className="oc-faq-drawer">
              {PAPER_FAQS.map((faq, i) => (
                <div key={i} className="oc-faq-item">
                  <button
                    className="oc-faq-question"
                    type="button"
                  ...
                  {openFaqIdx === i && (
                    <div className="oc-faq-answer">{faq.a}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="oc-toggle-header">
            <div className="oc-toggle-header-left">
              <svg viewBox="0 0 20 20" fill="none" width="20" height="20">
                <rect x="2" y="3" width="16" height="14" rx="2" stroke="var(--brand-petrol)" strokeWidth="1.3" />
                <path d="M6 8h8M6 11h5" stroke="var(--brand-petrol)" strokeWidth="1.2" strokeLinecap="round" />
              </svg>
              <div>
              ...
              {entries.map((entry, idx) => (
                <div key={idx} className="oc-special-entry">
                  <div className="oc-special-entry-header">
                    ...
                    <button className="oc-special-remove" onClick={() => removeEntry(idx)}>Remove</button>
                  </div>
                  ...
                  <div className="oc-field-row oc-field-row--2">
                    ...
                    <div className="input-group">
                      <label className="input-label">No. of Sheets</label>
                      <input type="number" className="input-field" min={1} max={config.sheets || 80} value={entry.sheets} readOnly />
                    </div>
                  </div>
                  ...
                  <div className="oc-field-row oc-field-row--2" style={{ marginTop: 'var(--space-4)' }}>
                    <div className="input-group">
                      ...
                      <input type="number" className="input-field" min={1} max={totalPages} value={entry.fromPage} onChange={e => updateEntry(idx, 'fromPage', e.target.value)} placeholder="e.g. 5" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">To Page</label>
                      <input type="number" className="input-field" min={entry.fromPage || 1} max={totalPages} value={entry.toPage} onChange={e => updateEntry(idx, 'toPage', e.target.value)} placeholder="e.g. 6" />
                    </div>
                  </div>

                  {entry.selectedPages.length > 0 && (
                    <div className="oc-page-pills-section">
                      <label className="input-label">Apply Special Paper To</label>
                      <div className="oc-page-pills">
                        {entry.selectedPages.map(p => (
                          <button key={p} className="oc-page-pill oc-page-pill--active" onClick={() => togglePage(idx, p)}>
                            Page {p}
                          </button>
                        ))}
                        <button className="oc-page-pill oc-page-pill--add" onClick={() => addPageToEntry(idx)}>
                          + Add page
                        </button>
                      </div>
                    </div>
      ...
      {config.paper && (
        <section className="oc-section">
          ...
          <div className="pdp-config-cards">
            ...
            >
              <div style={{ fontSize: 'var(--text-body-xs)', color: 'var(--warning)', fontWeight: 'var(--weight-semibold)', marginTop: 'var(--space-1)' }}>+15% surcharge</div>
            </ConfigCard>
          </div>
        </section>
      )}
